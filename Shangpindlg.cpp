...
void CShangpindlg::OnButadd() 
{
	...
	CString sql,ph;
	sql.Format("select * from mr.TB_SHANGPIN");
	ADOConn m_AdoConn;
...
void CShangpindlg::OnButmod() 
{
	...
	CString sql;
	sql.Format("update mr.TB_SHANGPIN set SPMC='%s',SPJM='%s',SPLB='%s',\
...
void CShangpindlg::OnButsave() 
{
	...
	CString sql;
	sql.Format("insert into mr.TB_SHANGPIN(SPBH,SPMC,SPJM,SPLB,SPDW,TXM,\
...
void CShangpindlg::OnButdel() 
{
	...
	UpdateData(TRUE);
	CString sql;
	sql.Format("delete from mr.TB_SHANGPIN where SPBH='%s'",m_Spbh);
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.ExecuteSQL((_bstr_t)sql);
	sql.Format("select * from mr.TB_DEPOT where SPBH='%s'",m_Spbh);
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	if(!m_AdoConn.m_pRecordset->adoEOF)
	{
...
void CShangpindlg::OnButquery() 
{
	...
	if(condition=="LIKE")
		sql.Format("select * from mr.TB_SHANGPIN where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_SHANGPIN where %s %s '%s'",
			field,condition,m_Text);
	m_Grid.DeleteAllItems();
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
