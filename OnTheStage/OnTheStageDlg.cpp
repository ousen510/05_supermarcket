...
void COnTheStageDlg::OnButtonadd() 
{
	...
	sql.Format("select * from mr.TB_SHANGPIN where %s='%s'",field,Text);
	ADOConn m_AdoConn;
...
void COnTheStageDlg::OnButtonok() 
{
	...
	{
		sql.Format("update mr.TB_DEPOT set SPSL=SPSL-%d where SPBH='%s'",
...
void COnTheStageDlg::Print()
{
	...
	sql.Format("select * from mr.TB_SELL where TO_DATE(XSSJ)=\
	...
	for(i=0;i<m_Grid.GetItemCount();i++)
	{
		...
		sql.Format("insert into mr.TB_SELL(XSPH,SPBH,SPMC,SPJM,SPLB,TXM,\
