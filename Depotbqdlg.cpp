...
BOOL CDepotbqdlg::OnInitDialog() 
{
	...
	CString sql;
	int i = 0;
	sql.Format("select * from mr.TB_BDEPOTQ");
...
void CDepotbqdlg::OnButquery() 
{
	...
	if(condition == "LIKE")
		sql.Format("select * from mr.TB_BDEPOTQ where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_BDEPOTQ where %s %s '%s'",
			field,condition,m_Text);
...
void CDepotbqdlg::OnButdel() 
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("delete from mr.TB_BDEPOTQ where SPBH='%s'",xsph);
