...
BOOL CDepotBackdlg::OnInitDialog() 
{
	...
	CString sql,ph;
	sql.Format("select distinct GYSBH,GYSMC from mr.TB_DEPOT");
	ADOConn m_AdoConn;
	...
	sql.Format("select * from mr.TB_DEPOT");
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	while(!m_AdoConn.m_pRecordset->adoEOF)
	{
...
void CDepotBackdlg::OnButsave() 
{
	...
	CString sql;
	sql.Format("insert into mr.TB_BDEPOTQ(SPBH,SPMC,SPJJ,THSL,THSJ,GYSBH,\
