...
void CDaydlg::OnQuery() 
{
	...
	int num=0;
	double spdj[1000];
	sql.Format("select distinct SPBH,SPMC,SPDJ from mr.TB_SELL where TO_DATE(XSSJ)=\
	...
	for(int j=0;j<num;j++)
	{
		sql.Format("select SUM(XSSL)AS ZSL from mr.TB_SELL\
		...
		if(!m_AdoConn.m_pRecordset->adoEOF)
		{
			...
			sum = sum + spdj[j]*zsl;
			sql.Format("select * from mr.TB_MONTH where MONTH='%s' and \
				SPBH='%s'",time.Format("%m月%Y年"),spbh[j]);	
			m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
			if(!m_AdoConn.m_pRecordset->adoEOF)
			{
				sql.Format("update mr.TB_MONTH set XSSL=XSSL+%d where SPBH='%s' and \
					MONTH='%s'",zsl,spbh[j],time.Format("%m月%Y年"));
			}
			else
			{
				sql.Format("insert into mr.TB_MONTH(SPBH,SPMC,SPDJ,XSSL,MONTH)values('%s',\
