...
void CUserdlg::OnButadd() 
{
	...
	CString sql;
	sql.Format("insert into mr.TB_USER (NAME,PWD,POPEDOM) values ('%s','%s','%s')",
...
void CUserdlg::AddToList()
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("select NAME from mr.TB_USER ");
...
void CUserdlg::OnButmod() 
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("update mr.TB_USER set NAME='%s',PWD='%s',POPEDOM='%s'\
...
void CUserdlg::OnButdel() 
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("delete from mr.TB_USER where NAME='%s' ",m_oName);
	m_AdoConn.ExecuteSQL((_bstr_t)sql);
	m_AdoConn.ExitConnect();
	m_List.DeleteAllItems();
	AddToList();
