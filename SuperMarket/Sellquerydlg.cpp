...
BOOL CSellquerydlg::OnInitDialog() 
{
	...
	CString sql;
	int i = 0;
	sql.Format("select * from mr.TB_SELL");
...
void CSellquerydlg::OnButquery() 
{
	...
	if(condition=="LIKE")
		sql.Format("select * from mr.TB_SELL where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_SELL where %s %s '%s'",
			field,condition,m_Text);
...
void CSellquerydlg::OnButdel() 
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("delete from mr.TB_SELL where XSPH = '%s'",xsph);
