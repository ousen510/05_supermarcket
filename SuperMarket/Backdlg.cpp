...
void CBackdlg::OnButadd() 
{
	...
	CString sql,ph;
	sql.Format("select * from mr.TB_BACK where TO_DATE(THRQ)=\
...
BOOL CBackdlg::OnInitDialog() 
{
	...
	CString sql,ph;
	sql.Format("select distinct XSPH from mr.TB_SELL");
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	while(!m_AdoConn.m_pRecordset->adoEOF)
...
void CBackdlg::OnButsave() 
{
	...
	m_Thrq.GetTime(time);
	CString sql;
	sql.Format("insert into mr.TB_BACK(THPH,XSPH,SPBH,SPMC,XSJE,CZY,THRQ)values(\
		'%s','%s','%s','%s',%f,'%s',TO_DATE('%s','MMDDYYYY'))",m_Thph,xsph,spbh,spmc,
		atof(xsje),m_UserName,time.Format("%m%d%Y"));
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.ExecuteSQL((_bstr_t)sql);
	sql.Format("select * from mr.TB_SELL where XSPH='%s' and SPBH='%s'",xsph,spbh);
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	if(!m_AdoConn.m_pRecordset->adoEOF)
	{
		int str;
		str = atoi((char*)(_bstr_t)m_AdoConn.m_pRecordset->GetCollect("XSSL"));
		sql.Format("update mr.TB_DEPOT set SPSL=SPSL+%d where SPBH='%s'",str,spbh);	
		m_AdoConn.ExecuteSQL((_bstr_t)sql);
		sql.Format("delete from mr.TB_SELL where XSPH='%s' and SPBH='%s'",xsph,spbh);
		m_AdoConn.ExecuteSQL((_bstr_t)sql);
	}
	m_AdoConn.ExitConnect();
	CDialog::OnCancel();
...
void CBackdlg::OnSelchangeCombo1() 
{
	...
	m_Xsph.GetLBText(m_Xsph.GetCurSel(),xsph);
	CString sql,ph;
	sql.Format("select * from mr.TB_SELL where XSPH='%s'",xsph);
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	while(!m_AdoConn.m_pRecordset->adoEOF)
