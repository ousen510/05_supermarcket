...
void CEmployees::AddToGrid()
{
	...
	CString sql;
	int i = 0;
	sql.Format("select * from mr.TB_EMPLOYEES");
...
void CEmployees::OnButsave() 
{
	...
	CString sql;
	sql.Format("insert into mr.TB_EMPLOYEES(YGBH,YGXM,YGXB,YGZW,YGXZ)\
...
void CEmployees::OnButmod() 
{
	...
	m_Ygzw.GetWindowText(ygzw);
	CString sql;
	sql.Format("update mr.TB_EMPLOYEES set YGXM='%s',YGXB='%s',YGZW='%s',\
...
void CEmployees::OnButdel() 
{
	...
	UpdateData(TRUE);
	CString sql;
	sql.Format("delete from mr.TB_EMPLOYEES where YGBH='%s'",m_Ygbh);
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.ExecuteSQL((_bstr_t)sql);
	m_AdoConn.ExitConnect();
...
void CEmployees::OnButquery() 
{
	...
	if(condition=="LIKE")
		sql.Format("select * from mr.TB_EMPLOYEES where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_EMPLOYEES where %s %s '%s'",
			field,condition,m_Text);
	m_Grid.DeleteAllItems();
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
