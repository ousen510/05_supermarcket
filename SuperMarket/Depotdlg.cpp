...
void CDepotdlg::OnButsave() 
{
	...
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	sql.Format("select * from mr.TB_DEPOT where SPBH='%s'");
	m_AdoConn.m_pRecordset = m_AdoConn.GetRecordSet((_bstr_t)sql);
	if(!m_AdoConn.m_pRecordset->adoEOF)
	{
		sql.Format("update mr.TB_DEPOT set SPSL=SPSL+%d,JHSJ=TO_DATE('%s',\
			'MMDDYYYY HH24:MI'),GYSBH='%s',GYSMC='%s' where SPBH='%s'",atoi(jhsl),
			time.Format("%m%d%Y %H:%M"),gysbh,gysmc,m_Spbh);
	}
	else
	{
		sql.Format("insert into mr.TB_DEPOT(SPBH,SPMC,SPJJ,SPSL,JHSJ,GYSBH,\
