...
BOOL CBackquerydlg::OnInitDialog() 
{
	...
	CString sql;
	int i = 0;
	sql.Format("select * from mr.TB_BACK");
...
void CBackquerydlg::OnButquery() 
{
	...
	if(condition=="LIKE")
		sql.Format("select * from mr.TB_BACK where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_BACK where %s %s '%s'",
			field,condition,m_Text);
...
void CBackquerydlg::OnButdel() 
{
	...
	m_AdoConn.OnInitADOConn();
	CString sql;
	sql.Format("delete from mr.TB_BACK where THPH = '%s'",thph);
