...
void CProvidedlg::OnButadd() 
{
	...
	CString sql,ph;
	sql.Format("select * from mr.TB_PROVIDE");
	ADOConn m_AdoConn;
...
void CProvidedlg::OnButsave() 
{
	...
	CString sql;
	sql.Format("insert into mr.TB_PROVIDE(GYSBH,GYSMC,GYSJC,GYSDZ,LXR,\
...
void CProvidedlg::OnButmod() 
{
	...
	CString sql;
	sql.Format("update mr.TB_PROVIDE set GYSMC='%s',GYSJC='%s',GYSDZ='%s',\
...
void CProvidedlg::OnButdel() 
{
	...
	UpdateData(TRUE);
	CString sql;
	sql.Format("delete from mr.TB_PROVIDE where GYSBH='%s'",m_Gysbh);
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
	m_AdoConn.ExecuteSQL((_bstr_t)sql);
	m_AdoConn.ExitConnect();
...
void CProvidedlg::OnButquery() 
{
	...
	if(condition=="LIKE")
		sql.Format("select * from mr.TB_PROVIDE where %s %s '%s%s%s'",
			field,condition,"%",m_Text,"%");
	else
		sql.Format("select * from mr.TB_PROVIDE where %s %s '%s'",
			field,condition,m_Text);
	m_Grid.DeleteAllItems();
	ADOConn m_AdoConn;
	m_AdoConn.OnInitADOConn();
